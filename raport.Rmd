...
```{r results='asis', warning=FALSE, message=FALSE, echo=F}
...
aktywa_st %>% 
  ...
  kable(escape = F, col.names = c("Aktywa", '2017', '2018', '2019', '2017', 
  ...
  column_spec(8:10, background = "#F0FCD9")
...
pasywa_st %>% 
  ...
  kable(escape = F, col.names = c("Pasywa", '2017', '2018', '2019', 
...
bold_r <- function(tabl) {
  return(na.omit(tabl %>% 
                   select(1) %>% 
                   mutate_at(vars(1), ~ ifelse(str_detect(.x, '^[ABP]'), row_number(), NA)) %>% 
                   unlist(., use.names=FALSE)))
}

italic_r <- function(tabl) {
  return(na.omit(aktywa %>% 
                   select(1) %>% 
                   mutate_at(vars(1), ~ ifelse(str_detect(.x, '^[VI]'), row_number(), NA)) %>% 
                   unlist(., use.names=FALSE)))
}
...
aktywa %>% 
  ...
  add_indent(setdiff(1:count(aktywa)[[1]], c(bold_r(aktywa), italic_r(aktywa))))
